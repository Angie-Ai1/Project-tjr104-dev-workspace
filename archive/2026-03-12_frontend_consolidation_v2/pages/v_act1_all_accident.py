...
def build_dynamic_rank_change_table(df, entity_col):
    if df.empty: return pd.DataFrame()
    yearly_df = df.groupby(['Year', entity_col])['pdi_score'].sum().reset_index()
    yearly_df = yearly_df.rename(columns={'pdi_score': 'pdi'})
    ...
    for y in display_years:
        prev_y = y - 1
        current_top5 = yearly_df[(yearly_df['Year'] == y) & (yearly_df['Rank'] <= 5)].sort_values('Rank')
        
        formatted_list = []
        for _, row in current_top5.iterrows():
        ...
        while len(formatted_list) < 5:
            formatted_list.append("-")
        result_dict[str(y)] = formatted_list
        
    res_df = pd.DataFrame(result_dict)
    if not res_df.empty:
        res_df.insert(0, '名次', [f"第 {i} 名" for i in range(1, 6)]) 
    return res_df

# ⭐ 新增：專為縣市排行榜設計的「事故總數」歷年排行變化
def build_dynamic_rank_change_table_by_acc(df, entity_col):
    if df.empty: return pd.DataFrame()
    yearly_df = df.groupby(['Year', entity_col])['accident_id'].count().reset_index()
    yearly_df = yearly_df.rename(columns={'accident_id': '事故總數'})
    yearly_df['Rank'] = yearly_df.groupby('Year')['事故總數'].rank(ascending=False, method='first')
...
def main():
    ...
    with col_right:
        if not df_filtered.empty:
            ...
            # ⭐ 修改：呼叫專為事故數量設計的函式，將歷年排行改為「事故總數」比較
            city_rank_table = build_dynamic_rank_change_table_by_acc(df_trend_base, 'nightmarket_city')
            mkt_rank_table = build_dynamic_rank_change_table(df_trend_base, 'nightmarket_name')
