...
def main():
    ...
    @media (max-width: 1400px) { div[data-testid="column"] { min-width: 450px !important; flex: 1 1 450px !important; margin-bottom: 20px; } }
    @media (max-width: 768px) { div[data-testid="column"] { min-width: 100% !important; flex: 1 1 100% !important; } }
    </style>
    ...
    with c_col1:
        ...
        region_idx = region_opts.index(def_region) if def_region in region_opts else 0
        sel_region = st.selectbox("區域", region_opts, index=region_idx, key="d_region", label_visibility="collapsed")
        
        city_opts = sorted(df_market[df_market['Region'] == sel_region]['City'].dropna().unique())
        city_idx = city_opts.index(def_city) if (sel_region == def_region and def_city in city_opts) else 0
        ...
        dist_idx = dist_opts.index(def_dist) if (sel_city == def_city and def_dist in dist_opts) else 0
        sel_dist = st.selectbox("行政區", dist_opts, index=dist_idx, key="d_dist", label_visibility="collapsed")

        m_opts = ["🔍 全台概覽 (預設)"] + sorted(df_market[(df_market['Region'] == sel_region) & (df_market['City'] == sel_city) & (df_market['AdminDistrict'] == sel_dist)]['MarketName'].dropna().unique())
        market_idx = m_opts.index(def_market) if (sel_dist == def_dist and def_market in m_opts) else 0
