...
import os
import ast
from shapely.geometry import Point, MultiPoint # 保留你原本引用的套件
...
# ==========================================
# 2. CloudSQL 處理邏輯 (夜市經緯度為中心點格式)
# ==========================================

def _fetch_from_mysql():
    """從 Cloud SQL (test_NM.nightmarkets) 讀取並清洗資料"""
    ...
    try:
        ...
        """
        把每個 item 用 day, time_val = item.split(':', 1) 拆成「星期 → 時間」，
...
def _fetch_from_csv(csv_path):
    ...
    try:
        ...
        # 【重要修正】這裡的縮排往左移了, 移出 format_schedule 函式外面, 因為要對整個 DataFrame 執行
