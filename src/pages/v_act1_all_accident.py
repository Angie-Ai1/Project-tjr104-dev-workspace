    ...
    with col_right:
        if not df_filtered.empty:
            ...
            if is_pdi_mode:
                ...
                mid_tag_html = f"""<div style="font-size: 12px; color: {mid_color}; background: {mid_bg}; padding: 2px 8px; border-radius: 4px; display: inline-block; font-weight: bold;">{sign}{mid_diff:{fmt}} ({mid_pct:+.2f}%) vs 國均</div>"""
        ...
        if not df_filtered.empty:
            ...
            for idx, (_, row) in enumerate(city_top5.reset_index(drop=True).iterrows()):
                ...
                if diff_val > 0:
                    nat_icon = "🔴"
                    nat_text = f"{nat_icon} 高於國均 {diff_val:{fmt}} {unit} <span style='font-size:10.5px; opacity:0.85;'>({diff_pct:+.2f}%)</span>"
                else:
                    nat_icon = "🟢"
                    nat_text = f"{nat_icon} 低於國均 {abs(diff_val):{fmt}} {unit} <span style='font-size:10.5px; opacity:0.85;'>({diff_pct:+.2f}%)</span>"
